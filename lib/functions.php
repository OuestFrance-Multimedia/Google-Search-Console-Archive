 ...
 *
 * @param array $interval Interval array ('base' & 'compare')
 * @return string
 */
function html_interval($interval) {
    ...
    if ($interval['base']['from'] != $interval['base']['to']) {
        $string = ' from ' . iconv('ISO-8859-1', 'UTF-8', strftime('%e %B %Y', strtotime($interval['base']['from']))) .
                  ' to ' . iconv('ISO-8859-1', 'UTF-8', strftime('%e %B %Y', strtotime($interval['base']['to'])));
    } else {
    ...
    if (isset($interval['compare'])) {
        ...
        if ($interval['compare']['from'] != $interval['compare']['to']) {
            $string .= ' period from ' . iconv('ISO-8859-1', 'UTF-8', strftime('%e %B %Y', strtotime($interval['compare']['from']))) .
                       ' to ' . iconv('ISO-8859-1', 'UTF-8', strftime('%e %B %Y', strtotime($interval['compare']['to'])));
        } else {
